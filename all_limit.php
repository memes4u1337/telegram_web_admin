<?php
// all_limit.php — список и редактирование лимитов (search_limits)
...
/* ---------- перевод (тот же стек, что в all_tariffs.php) ---------- */
...
/* ---------- строгая проверка ADMIN как в users.php / all_tariffs.php ---------- */
...
try {
    ...
    error_log('all_limit.php admin fetch error: ' . $e->getMessage());
}
...
/* ---------- работа с лимитами поиска (search_limits) ---------- */

function normalizeLimitRow(array $row): array {
    ...
    if (($lab_ru === null || $lab_en === null || $lab_es === null) && $duration_days > 0) {
        $labels = makeDurationLabels($duration_days, 'days');
...
function fetchAllLimits(PDO $pdo): array {
    $limits = [];
    try {
        $st = $pdo->query("
            SELECT
                ...
                is_active
            FROM search_limits
            ORDER BY sort_order ASC, price ASC, search_limit ASC, code ASC
        ");
        while ($row = $st->fetch(PDO::FETCH_ASSOC)) {
            $limits[] = normalizeLimitRow($row);
        }
    } catch (Throwable $e) {
        error_log('all_limit.php fetchAllLimits error: ' . $e->getMessage());
    }
    return $limits;
}

function fetchLimitByCode(PDO $pdo, string $code): ?array {
    try {
        $st = $pdo->prepare("
            SELECT
                code,
        ...
        if ($row = $st->fetch(PDO::FETCH_ASSOC)) {
            return normalizeLimitRow($row);
        }
    } catch (Throwable $e) {
        error_log('all_limit.php fetchLimitByCode error: ' . $e->getMessage());
    }
    return null;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $dbStatusOk) {
    ...
    if ($ajax === 'translate_description') {
        ...
        try {
            ...
            error_log('all_limit.php translate_description error: ' . $e->getMessage());
            echo json_encode([
                'ok'    => false,
    ...
    // обновление лимита поиска
    if ($ajax === 'update_limit') {
        ...
        $existing = fetchLimitByCode($pdo, $code);
        if (!$existing) {
            echo json_encode([
                'ok'    => false,
        ...
        if ($durationDays <= 0) {
            $errors[] = 'Укажите срок действия лимита (дней).';
        }

        if ($searchLimit < 0) {
            $errors[] = 'Лимит запросов не может быть меньше нуля.';
        }

        // проверка уникальности title, кроме текущего code
        if ($title !== '') {
            try {
                $st = $pdo->prepare("SELECT code FROM search_limits WHERE title = :t AND code <> :c LIMIT 1");
        ...
        $price = (float)str_replace(',', '.', $priceRaw);
        $labels = makeDurationLabels($durationDays, 'days');
        ...
        try {
            ...
            $updated = fetchLimitByCode($pdo, $code);
            ...
            error_log('all_limit.php update_limit error: ' . $e->getMessage());
            echo json_encode([
                'ok'    => false,
                'error' => 'db_error',
                'msg'   => 'Ошибка при сохранении лимита: ' . $e->getMessage(),
            ], JSON_UNESCAPED_UNICODE);
        }
        exit;
    }
...
$limits = $dbStatusOk ? fetchAllLimits($pdo) : [];
...
<head>
    <meta charset="UTF-8">
    <title>Лимиты поиска (search_limits)</title>
...
<div class="container">
    <div class="header">
        <div class="page-title">
            <h1>Все лимиты поиска</h1>
            <p>Таблица <code>search_limits</code></p>
        </div>
    ...
    <div class="tariffs-card">
        <div class="tariffs-header-row">
            <div>
                <div class="tariffs-header-row-title">Все лимиты</div>
            </div>
        ...
        <?php if ($dbStatusOk): ?>
            <div class="tariffs-toolbar">
                <div class="toolbar-left">
                    <?php if (count($limits)): ?>
                        Найдено лимитов: <strong><?= count($limits) ?></strong>
                    <?php else: ?>
                        Лимиты не найдены.
                    <?php endif; ?>
                </div>
            ...
            <?php if (!count($limits)): ?>
                <div class="empty-state">
                    ...
                    В таблице <code>search_limits</code> пока нет лимитов.
                </div>
            <?php else: ?>
                <div id="limits-grid" class="tariffs-grid">
                    <?php foreach ($limits as $p): ?>
                        <div class="tariff-card"
                             data-code="<?= h($p['code']) ?>"

                             data-title="<?= h($p['title']) ?>"
                             data-search_limit="<?= (int)$p['search_limit'] ?>"
                             ...
                             data-duration_days="<?= (int)$p['duration_days'] ?>"
                        ...
                        >
                            ...
                            <div class="tariff-main-row">
                                ...
                                <div class="tariff-limit">
                                    До <?= (int)$p['search_limit'] ?> поисков за период
                                </div>
                            </div>
    ...
    <div class="modal">
        <div class="modal-header">
            <div class="modal-header-left">
                <div class="modal-title" id="modal-title">Редактирование лимита</div>
        ...
        <div class="modal-body">
            <form id="limit-form">
                <input type="hidden" name="ajax" value="update_limit">
                <input type="hidden" name="code" id="limit-code-input">
                <input type="hidden" name="is_active" id="limit-is-active-hidden" value="1">
                ...
                <div class="field">
                    ...
                    <input type="text"
                           class="input"
                           name="title"
                           id="limit-title-input"
                           maxlength="100"
                           required>
                    <div class="inline-hint">
                        Название, которое увидит пользователь. Не должно дублировать другие лимиты.
                    </div>
                </div>
                ...
                <div class="field flex-row">
                    <div>
                        <label class="field-label">
                            <span>Лимит поисков за период</span>
                        </label>
                        <input type="number"
                               class="input"
                               name="search_limit"
                               id="limit-search-limit-input"
                               min="0"
                               required>
                    </div>
                    <div>
                ...
                <div class="field">
                    <label class="field-label">
                        <span>Период действия лимита (дней)</span>
                    </label>
                    <div class="flex-row">
                        <div>
                            <input type="number"
                    ...
                    <div class="switch-row">
                        <div>
                            <div class="field-label">
                                <span>Статус лимита</span>
                            </div>
                            <div class="inline-hint">
                                Если выключить — лимит не будет предлагаться новым пользователям.
                            </div>
                        </div>
                        <label class="switch">
                            <input type="checkbox" id="limit-is-active-input">
        ...
        <div class="modal-footer">
            <div>Все изменения сохраняются напрямую в таблицу <code>search_limits</code>.</div>
            <button type="button" class="btn-save" id="btn-save-limit">
...
(function(){
    ...
    const modalOverlay = document.getElementById('limitModal');
    const modalClose = document.getElementById('modal-close');

    const form = document.getElementById('limit-form');
    const btnSave = document.getElementById('btn-save-limit');
    const modalErrors = document.getElementById('modal-errors');

    const titleInput = document.getElementById('limit-title-input');
    const codeHidden = document.getElementById('limit-code-input');
    const modalCode = document.getElementById('modal-code');
    const modalTitle = document.getElementById('modal-title');

    const searchLimitInput = document.getElementById('limit-search-limit-input');
    const priceInput = document.getElementById('limit-price-input');
    const sortOrderInput = document.getElementById('limit-sort-order-input');
    const durationDaysInput = document.getElementById('limit-duration-days-input');

    const descRu = document.getElementById('limit-desc-ru');
    const descEn = document.getElementById('limit-desc-en');
    const descEs = document.getElementById('limit-desc-es');

    const durationPreviewRow = document.getElementById('duration-preview-row');

    const isActiveCheckbox = document.getElementById('limit-is-active-input');
    const isActiveHidden = document.getElementById('limit-is-active-hidden');
    ...
    function openModalFromCard(card){
        ...
        const searchLimit    = card.getAttribute('data-search_limit') || '0';
        const price          = card.getAttribute('data-price') || '0';
        const sortOrder      = card.getAttribute('data-sort_order') || '0';
        const durationDays   = card.getAttribute('data-duration_days') || '0';
        ...
        modalTitle.textContent = 'Редактирование лимита';

        titleInput.value = title;
        searchLimitInput.value = searchLimit;
        priceInput.value = price;
        sortOrderInput.value = sortOrder;
        durationDaysInput.value = durationDays;
    ...
    function getDurationLabelsDays(value){
    ...
    function updateDurationPreview(){
        ...
        const v = parseInt(durationDaysInput.value, 10) || 0;
        ...
        const labels = getDurationLabelsDays(v);
    ...
    // превью периода
    durationDaysInput && durationDaysInput.addEventListener('input', updateDurationPreview);
        ...
        btnSave.addEventListener('click', function(){
            ...
            fetch('all_limit.php', {
                ...
                .then(data => {
                    ...
                    if (limit && grid) {
                        const card = grid.querySelector('.tariff-card[data-code="' + limit.code + '"]');
                        if (card) {
                            card.setAttribute('data-title', limit.title || '');
                            card.setAttribute('data-search_limit', limit.search_limit || 0);
                            card.setAttribute('data-price', limit.price || 0);
                            card.setAttribute('data-description_ru', limit.description_ru || '');
                            card.setAttribute('data-description_en', limit.description_en || '');
                            card.setAttribute('data-description_es', limit.description_es || '');
                            card.setAttribute('data-duration_days', limit.duration_days || 0);
                            card.setAttribute('data-duration_label_ru', limit.duration_label_ru || '');
                            card.setAttribute('data-duration_label_en', limit.duration_label_en || '');
                            card.setAttribute('data-duration_label_es', limit.duration_label_es || '');
                            card.setAttribute('data-sort_order', limit.sort_order || 0);
                            card.setAttribute('data-is_active', limit.is_active ? '1' : '0');

                            const nameEl = card.querySelector('.tariff-name');
                            if (nameEl) nameEl.textContent = limit.title || '';

                            const periodSub = card.querySelector('.tariff-main-row .tariff-price span');
                            const priceEl = card.querySelector('.tariff-price');
                            if (priceEl) {
                                const priceVal = (parseFloat(limit.price || 0)).toFixed(2);
                                ...
                                if (periodSub) {
                                    periodSub.textContent = ' / ' + (limit.duration_label_en || 'period');
                                }
                            }

                            const limitEl = card.querySelector('.tariff-limit');
                            if (limitEl) {
                                limitEl.textContent = 'До ' + (limit.search_limit || 0) + ' поисков за период';
                            }

                            const descEl = card.querySelector('.tariff-desc');
                            if (descEl) {
                                descEl.textContent = limit.description_ru || limit.description || 'Описание не задано';
                            }

                            const sortEl = card.querySelector('.tariff-sort');
                            if (sortEl) {
                                sortEl.textContent = 'sort: ' + (limit.sort_order || 0);
                            }
                    ...
                    showFlash('success', data.msg || 'Лимит сохранён');
                    closeModal();
                    if (filterActive) {
                        filterActive.dispatchEvent(new Event('change'));
                    }
